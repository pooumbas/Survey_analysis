...
def get_xy(file,num):
    ...
    with open (file) as r:
        for ii in r:
            a=json.loads(ii)
            ys=-a['LeInput']["EpRequest"]["LocEst"][0]['Point']['Y']
            xs=a['LeInput']["EpRequest"]["LocEst"][0]['Point']['X']
            secx=a['LeInput']['EpRequest']["LocEst"][5]['Point']['X']
            secy=a['LeInput']['EpRequest']["LocEst"][5]['Point']['Y']
            x1=a['StaticX']
            y1=(-a['StaticY'])
            if switch == True:
                delta += 1
...
def save_metrics(interx, intery, onex, oney, pfx, pfy):
    ...
    metrics=['DR Mean Error:', 'DR STDV Error:', 'DR Estimates > 3 Meter Error:', 'Particle Filter Mean:', 'Particle Filter STDV:','Particle Filter Estimates > 3 Meter Error:']
    metrics2=[ second_mean, second_STD,three_meter_one, pfmean, pfSTD,three_meter_PF]
    print('Saving Metrics To metrics.txt...')
    with open ('metrics.txt', 'a') as w:
        w.write('\n'*2+'__________Error Metrics__________' + '\n' +'\n')
        index=0
...
def save_fig(boo,plot_path, save_name):
    ...
    sliced=len(file_name)-1
    save_name=file_name[:sliced]+new_num+file_ext
    plt.savefig(save_name)
def format_name():
    dirs=os.listdir()
    maximum=[]
    for ii in dirs:
        ...
        if file_ext=='.png':
            ...
            maximum.append(int(file_name[index]))
    if len(maximum)==0:
        maximum.append(0)
    new_num=str(max(maximum)+1)
    return new_num
